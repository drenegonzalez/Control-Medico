...
# --- CAMBIO DE CONTRASEÑA PARA USUARIO LOGUEADO ---
@app.post("/usuarios/me/cambiar-password", tags=["Autenticación"])
def cambiar_mi_password(
    ...
    if len(datos.password_nueva.strip()) < 4:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="La nueva contraseña debe tener al menos 4 caracteres."
        )

    current_user.hashed_password = hash_password(datos.password_nueva)
    db.commit()
